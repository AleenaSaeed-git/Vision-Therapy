...
class VisionTherapyApp(App):
    ...
    def build(self):
        ...
        if initialize_fcm():
            ...
            fcm_client.on_message(self.handle_fcm_message)
        else:
    ...
    def handle_fcm_message(self, message):
        ...
        try:
            print(f"📱 Received FCM message: {message}")
            # Show notification using plyer
            from plyer import notification
            notification.notify(
                title=message.get('notification', {}).get('title', 'Vision Therapy'),
                message=message.get('notification', {}).get('body', 'New message'),
                timeout=10
            )
        except Exception as e:
            print(f"❌ Error handling FCM message: {str(e)}")
